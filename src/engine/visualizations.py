...
def plot_batch_summary(agg: dict) -> str:
    """Per-category mean markers with 95% bootstrap CI bars for the 4
    proven discriminative metrics.

    Deliberately does NOT render strip/jitter points: this function only
    receives summary statistics, and an earlier version synthesized fake
    "observations" from N(estimate, CI-width) to decorate the plot — a
    misrepresentation in a measurement tool. What is drawn now is exactly
    what is known at this layer: the estimate, its CI, and n."""
    ...
    for ax, metric, title in zip(axes, plot_metrics, titles):
        ...
        for ci, cat in enumerate(available):
            ...
            # Sample size beside the marker
            ax.text(ci + 0.12, est, f"n={n}", va="center", fontsize=10,
                    color=TEXT_MUTED)

        ax.set_xticks(range(len(available)))
        ax.set_xticklabels([c.capitalize() for c in available], fontsize=12)
        ax.tick_params(axis="x", length=0)
        ax.set_xlim(-0.5, len(available) - 0.3)
