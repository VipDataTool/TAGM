...
def _clip_common(trajs):
    """Clip a list of trajectories to their shortest common length.

    Element-wise means assume equal lengths; a session mixing models or
    partially-computed results is ragged and numpy 2.x raises on a ragged
    mean. Same remedy the changelog applied to discriminative_sublayers,
    now shared by every trajectory-averaging plot.
    """
    if not trajs:
        return trajs, 0
    n = min(len(t) for t in trajs)
    return [t[:n] for t in trajs], n
...
def plot_trajectory_overlay(results: list) -> str:
    ...
    n = min(len(r["amplitude_normalized"]) for r in has_traj)
...
def plot_difference_from_benign(results: list) -> str:
    ...
    # One common length across ALL plotted trajectories (benign baseline
    # and every category), so subtraction below is shape-safe too.
    common = min(len(r["amplitude_normalized"]) for r in has_traj)
    benign, _ = _clip_common([t[:common] for t in benign])
    ...
    for cat, trajs in cats.items():
        trajs, _ = _clip_common([t[:common] for t in trajs])
...
def plot_discriminative_sublayers(results: list, top_n: int = None) -> str:
    ...
    # Trajectories must share a length to average element-wise (see
    # _clip_common). Clip both groups to one shared length.
    all_clipped, common_len = _clip_common(benign + adversarial)
    benign_mean = np.mean(all_clipped[:len(benign)], axis=0)
    adv_mean = np.mean(all_clipped[len(benign):], axis=0)
...
def plot_key_scatters(results: list) -> str:
    ...
    for ax, (xkey, ykey, xlabel, ylabel) in zip(axes, panels):
        ...
        for cat in CAT_ORDER:
            pts = [(r.get(xkey), r.get(ykey)) for r in results
                   if r.get("category") == cat
