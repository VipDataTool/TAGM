...
def export_session_split(session, path: Union[str, Path],
    ...
    # Detect embedding dimensionality. Scan pages until a result that
    # actually carries embeddings is found — probing only result[0]
    # silently exported empty embedding CSVs whenever the first result
    # happened to lack them.
    n_dims = 0
    _scan = 0
    while n_dims == 0 and _scan < n_total:
        page = session.get_results_page(_scan, page_size)
        if not page:
            break
        for r in page:
