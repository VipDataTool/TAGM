...
from misc.old.tagm.core.deltas.store import DeltaStore, DeltaStoreMetadata
...
def compute_deltas_from_disk(
    ...
    def _key_wanted(key: str) -> bool:
        """True if this weight key is a projection and (if filtering) in scope."""
    ...
    def _process_file(fpath: Path, keys_to_extract: list[str]) -> None:
        """Read a set of tensors from a single safetensors file, compute deltas."""
        with safe_open(str(fpath), framework="pt") as f:
            ...
            for key in keys_to_extract:
                ...
                # Ensure compatible dtypes before subtraction
                if inst_tensor.dtype != dtype:
                    inst_tensor = inst_tensor.to(dtype)
                delta = inst_tensor - base_tensor
                parsed = adapter.parse_projection_key(key)
                if parsed is None:
                    # Defensive: should not happen because _key_wanted filtered
    ...
    # ── Dispatch based on sharded vs single-file layout ─────────────
    if index_path.exists():
        with open(index_path) as f:
            index = json.load(f)
        weight_map = index.get("weight_map", {})

        # Group wanted keys by shard file so we open each file once
    ...
    # ── Cleanup instruct state_dict reference ───────────────────────
    del inst_sd
    gc.collect()
