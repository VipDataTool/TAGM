...
from misc.old.tagm.engine.analyzer import Analyzer
from misc.old.tagm.engine.result import result_to_dict
from misc.old.tagm.engine import config as engine_config
from misc.old.tagm.engine.session import Session
from misc.old.tagm.engine.app_core import (
    AppState, state,
...
from misc.old.tagm.engine.modules import ModuleRunner
from misc.old.tagm.core.cache import Cache
from misc.old.tagm.service.events import broker
...
async def add_model(id: str = Form(...), name: str = Form(...),
    ...
    from misc.old.tagm.core.db import get_db
...
async def hep_status():
    ...
    from misc.old.tagm.engine import config as engine_config
    from misc.old.tagm.core.cache import system_resources, hf_cache_size
    from pathlib import Path
...
async def init_hep(request: Request):
    ...
    """
    from misc.old.tagm.engine import config as engine_config
    from misc.old.tagm.core.cache import clear_hf_cache, clear_mmap_deltas, system_resources
...
async def deactivate_hep():
    ...
    from misc.old.tagm.engine import config as engine_config
    from misc.old.tagm.core.cache import clear_mmap_deltas
...
async def probe_apply(request: Request):
    ...
    def _embed_worker():
        try:
            from misc.old.tagm.probes.io import embed_and_activate_probe_set
...
async def probe_status():
    _project_root = _PACKAGE_DIR.parent
    from misc.old.tagm.probes.io import get_active_probe_set, load_probes
...
async def export_session(request: Request):
    ...
    from misc.old.tagm.service.export import export_session_split
...
def _render_batch_plot(plot_key: str, results: list) -> bytes | None:
    ...
    if plot_key in ("batch_summary", "separability"):
        ...
        from misc.old.tagm.engine.statistics import aggregate_batch
        from misc.old.tagm.engine.visualizations import plot_batch_summary, plot_separability
    ...
    if func_name:
        import misc.old.tagm.engine.comparative as comp
...
async def get_plot(plot_key: str):
    ...
    try:
        ...
        from misc.old.tagm.service.plots import render_plot
...
async def chat(request: Request):
    from misc.old.tagm.service.chat import generate_chat_response
    if state.pipeline is None or not state.pipeline.loaded:
        return {"ok": False, "error": "No model loaded."}
    try:
        body = await request.json()
