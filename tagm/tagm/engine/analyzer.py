...
from tagm.engine.hooks import ActivationCapture
from tagm.engine.result import (
...
class Analyzer:
    ...
    def analyze_prompt(
        ...
        need_attn_output = full_capture or engine_config.get("attention_weighted_pool")
    ...
    def _compute_ltp(self, logits, tokens, input_ids,
                     k=8, layer_strategy="signal", svd_rank=0,
        ...
        """
        from tagm.engine.ltp import compute_ltp
        return compute_ltp(
    ...
    def _compute_sfd(self, seq_len: int) -> Optional[SFDResult]:
        ...
        from tagm.engine.sfd import compute_sfd, precompute_sfd_cache
    ...
    def _compute_rank_displacement(self, result: PromptResult):
        ...
        from tagm.engine.sfd import compute_rank_displacement
        try:
    ...
    def _compute_behavioral_comparison(self, result: PromptResult,
        ...
        with torch.no_grad():
            ...
            # Per-token base counterfactuals
            ...
            for i in range(base_logits_full.shape[0]):
                ...
                topk_result = torch.topk(base_logits_full[i], ltp_k + overfetch)
                ...
                for j, tid in enumerate(topk_result.indices.tolist()):
                    if tid != chosen_id and len(alts) < ltp_k:
                        alts.append((self.tokenizer.decode(tid).strip(),
                                     round(probs[j].item(), precision)))
                base_cf.append(alts)
