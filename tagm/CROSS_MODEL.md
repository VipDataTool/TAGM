...
| `n_negative_tokens / seq_len` | Sign distribution — interpretable as a fraction. |
| `ltp.mean_M / ltp.mean_V` | Magnitude / variance of *unit-direction-projected* tension. Lateral-tension geometry was always in unit-vector space. |
| `ltp.max_prc`, `n_directional / seq_len` | Concentration measures; dimensionless. |
| `ltp.profile_shapes` distribution | Counts of steep/flat/inverted as fractions of total tokens. |
| `sfd.density_mean`, `density_max`, `density_p90` | Density is *per-token erank divided by global erank* — normalized within-model already. |
| `rank_displacement.mean_tau` | Kendall τ ∈ [-1, 1]. Pure correlation. |
| `rank_displacement.mean_overlap` | Jaccard fraction. |
| `rank_displacement.replacement_ratio` | Mass fraction. |
...
| `kl_divergence` | Compute it on a small subsample only (see KL OOM warning); then look at *category-level differences*, not raw KL values |
